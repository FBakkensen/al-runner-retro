# AL Runner agent-workflow retrospective

Thirty-day retrospective (2026-08-10 to 2026-09-09) on the multi-agent workflow around
[StefanMaron/BusinessCentral.AL.Runner](https://github.com/StefanMaron/BusinessCentral.AL.Runner)
and its AL-language corpus. Rendered report: **https://fbakkensen.github.io/al-runner-retro/**

- `index.html` — the report (self-contained; every finding links to its issue, PR or commit)
- `prompt-for-maintainer.md` — a prompt that reproduces the same analysis over the other operator's own session transcripts, with this run's numbers for comparison

Read-only analysis of public GitHub data plus one operator's local session transcripts (counts and categories only). Nothing was filed, labelled or edited on GitHub as part of it.
